...
public record InMemoryRepository(Map<String, Account> accounts) implements Repository {
  ...
  // accountIdGenerator.encode(accountCounter.incrementAndGet()));
  //    return formatted;
  //  }
  //
  //  @Override
  ...
  public Either<Error, Account> create(CreateAccountRequest request) {
    return Either.right(new Account(accountCounter.incrementAndGet(), "", request.name()));
  }
}
